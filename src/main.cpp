...
const string strMessageMagic = "LuckyBlocks Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 9999)
    {
        nSubsidy = 50 * COIN; //~475000
    }

    else if(pindexBest->nHeight < 10000)
    {
        nSubsidy = 0 * COIN; //0.1 BTC or 0.5%
    }
    ...
    {
        nSubsidy = 0 * COIN; //0.2 BTC or 1%
    }

    else if(pindexBest->nHeight < 29999)
    {
        nSubsidy = 150 * COIN; //~1500000
    }

    else if(pindexBest->nHeight < 30000)
    {
        nSubsidy = 0 * COIN; //0.3 BTC or 1.5%
    }
    ...
    {
        nSubsidy = 0 * COIN; //0.4 BTC or 2%
    }

    else if(pindexBest->nHeight < 49999)
    {
        nSubsidy = 250 * COIN; //~2500000
    }

    else if(pindexBest->nHeight < 50000)
    {
        nSubsidy = 0 * COIN; //0.5 BTC or 2.5%
    }
    ...
    {
        nSubsidy = 0 * COIN; //1 BTC or 5%
    }

    else if(pindexBest->nHeight < 69999)
    {
        nSubsidy = 350 * COIN; //~3500000
    }

    else if(pindexBest->nHeight < 70000)
    {
        nSubsidy = 0 * COIN; //1.5 BTC or 7.5%
    }
    ...
    {
        nSubsidy = 0 * COIN; //4 BTC or 20%
    }

    else if(pindexBest->nHeight > 80001)
    {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LuckyBlocks", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "13th January 2016 - Now You Can Easily Exchange Bitcoin for Gold Bullion";
        CTransaction txNew;
        txNew.nTime = 1452704990;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1452704990;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2214878;
		if(fTestNet)
        {
            block.nNonce   = 2214878;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8b8053d9a3adff8d4a11a26d91641f15ab0abc567cbe5e20de364c8fcf8b2f51"));
        block.print();
    ...
    string strPubKey = "04e8064debf789f7119498f152e8fa80162a0606068c41cfea81cebe10c673c8ccf271ee2347dff740a352e1216cfb0c41ea9fe26d4525811b018008df418ee0a2";
